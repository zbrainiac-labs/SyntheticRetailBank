...
DEFINE TABLE {{ db }}.{{ loa_raw }}.LOAI_RAW_TB_EMAIL_INBOUND_LOAN_EXTRACT_FLAT (
 ...
 EXISTING_DEBTS_MONTHLY NUMBER(18,2),
 CREDIT_SCORE INT,
 ...
 LTV_RATIO_PCT NUMBER(5,2),
 DTI_RATIO_PCT NUMBER(5,2),

 EXTRACTION_SUCCESS BOOLEAN,
 RAW_EXTRACTED_DATA VARIANT
)
CHANGE_TRACKING = TRUE
...
AS
INSERT INTO {{ loa_raw }}.LOAI_RAW_TB_EMAIL_INBOUND_LOAN_EXTRACT (
 FILE_NAME,
 FILE_TIMESTAMP,
 EXTRACTED_DATA,
...
SELECT
 ...
 TO_FILE('@LOAI_RAW_ST_EMAIL_INBOUND', RELATIVE_PATH),
 (SELECT schema_json FROM {{ loa_raw }}.LOAI_RAW_TB_EMAIL_INBOUND_LOAN_SCHEMA_CONFIG)
 ) AS EXTRACTED_DATA,
 CURRENT_TIMESTAMP() AS EXTRACTION_TIMESTAMP
FROM DIRECTORY(@LOAI_RAW_ST_EMAIL_INBOUND)
WHERE RELATIVE_PATH LIKE '%mortgage%'
 AND RELATIVE_PATH LIKE '%_internal.txt'
 AND NOT EXISTS (
 SELECT 1 FROM {{ loa_raw }}.LOAI_RAW_TB_EMAIL_INBOUND_LOAN_EXTRACT e
...
AS
INSERT INTO {{ loa_raw }}.LOAI_RAW_TB_EMAIL_INBOUND_LOAN_EXTRACT_FLAT (
 FILE_NAME,
 FILE_TIMESTAMP,
 EXTRACTION_TIMESTAMP,
 DOCUMENT_TYPE,
...
FROM {{ loa_raw }}.LOAI_RAW_TB_EMAIL_INBOUND_LOAN_EXTRACT s
WHERE NOT EXISTS (
 SELECT 1 FROM {{ loa_raw }}.LOAI_RAW_TB_EMAIL_INBOUND_LOAN_EXTRACT_FLAT f
