...
DEFINE TABLE {{ db }}.{{ crm_raw }}.ACCI_RAW_TB_ACCOUNTS (
    ...
    CONSTRAINT PK_{{ crm_raw }}.ACCI_RAW_TB_ACCOUNTS PRIMARY KEY (ACCOUNT_ID)
)
CHANGE_TRACKING = TRUE
COMMENT = 'Account master data table supporting multi-currency retail banking operations. Each customer can have multiple accounts of different types. Investment accounts are used for equity trading settlement. FK to {{ crm_raw }}.CRMI_RAW_TB_CUSTOMER removed due to SCD Type 2 composite PK.';
...
AS
    COPY INTO {{ crm_raw }}.ACCI_RAW_TB_ACCOUNTS (ACCOUNT_ID, ACCOUNT_TYPE, BASE_CURRENCY, CUSTOMER_ID, STATUS)
    FROM @{{ crm_raw }}.ACCI_RAW_ST_ACCOUNTS
...
AS
    CALL CRMI_RAW_SP_CLEANUP_STAGE_KEEP_LAST_N('{{ crm_raw }}.ACCI_RAW_ST_ACCOUNTS', 5);
