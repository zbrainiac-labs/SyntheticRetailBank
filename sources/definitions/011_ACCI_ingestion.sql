...
DEFINE TASK {{ db }}.{{ crm_raw }}.ACCI_RAW_TK_CLEANUP_AFTER_LOAD_ACCOUNTS
    USER_TASK_MANAGED_INITIAL_WAREHOUSE_SIZE = 'XSMALL'
    COMMENT = 'Automated stage cleanup AFTER {{ db }}.{{ crm_raw }}.account data load. Keeps last 5 files to manage storage costs.'
    AFTER {{ db }}.{{ crm_raw }}.ACCI_RAW_TK_LOAD_ACCOUNTS
AS
    CALL CRMI_RAW_SP_CLEANUP_STAGE_KEEP_LAST_N('ACCI_RAW_ST_ACCOUNTS', 5);
