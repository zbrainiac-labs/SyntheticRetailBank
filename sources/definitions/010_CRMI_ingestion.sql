...
DEFINE TABLE {{ db }}.{{ crm_raw }}.CRMI_RAW_TB_CUSTOMER_STATUS (
    ...
    CONSTRAINT FK_CRMI_RAW_TB_CUSTOMER_STATUS__CRMI_RAW_TB_CUSTOMER_EVENT FOREIGN KEY (LINKED_EVENT_ID) REFERENCES CRMI_RAW_TB_CUSTOMER_EVENT (EVENT_ID)
)
CHANGE_TRACKING = TRUE
...
DEFINE TASK {{ db }}.{{ crm_raw }}.CRMI_RAW_TK_CLEANUP_AFTER_LOAD_CUSTOMERS
    USER_TASK_MANAGED_INITIAL_WAREHOUSE_SIZE = 'XSMALL'
    COMMENT = 'Automated stage cleanup AFTER {{ db }}.{{ crm_raw }}.customer data load. Keeps last 5 files to manage storage costs.'
...
DEFINE TASK {{ db }}.{{ crm_raw }}.CRMI_RAW_TK_CLEANUP_AFTER_LOAD_ADDRESSES
    USER_TASK_MANAGED_INITIAL_WAREHOUSE_SIZE = 'XSMALL'
    COMMENT = 'Automated stage cleanup AFTER {{ db }}.{{ crm_raw }}.address data load. Keeps last 5 files to manage storage costs.'
...
DEFINE TASK {{ db }}.{{ crm_raw }}.CRMI_RAW_TK_CLEANUP_AFTER_LOAD_EXPOSED_PERSON
    USER_TASK_MANAGED_INITIAL_WAREHOUSE_SIZE = 'XSMALL'
    COMMENT = 'Automated stage cleanup AFTER {{ db }}.{{ crm_raw }}.PEP data load. Keeps last 5 files to manage storage costs.'
