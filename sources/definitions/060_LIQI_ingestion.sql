...
AS
    COPY INTO LIQI_RAW_TB_HQLA_HOLDINGS (
...
AS
    COPY INTO LIQI_RAW_TB_DEPOSIT_BALANCES (
...
DEFINE TASK {{ db }}.{{ rep_raw }}.LIQI_RAW_TK_CLEANUP_AFTER_LOAD_HQLA_HOLDINGS
    USER_TASK_MANAGED_INITIAL_WAREHOUSE_SIZE = 'XSMALL'
    COMMENT = 'Automated stage cleanup AFTER {{ db }}.{{ rep_raw }}.HQLA holdings data load. Keeps last 5 files to manage storage costs.'
...
DEFINE TASK {{ db }}.{{ rep_raw }}.LIQI_RAW_TK_CLEANUP_AFTER_LOAD_DEPOSIT_BALANCES
    USER_TASK_MANAGED_INITIAL_WAREHOUSE_SIZE = 'XSMALL'
    COMMENT = 'Automated stage cleanup AFTER {{ db }}.{{ rep_raw }}.deposit balances data load. Keeps last 5 files to manage storage costs.'
