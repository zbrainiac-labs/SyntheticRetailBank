...
DEFINE TABLE {{ db }}.{{ eqt_raw }}.EQTI_RAW_TB_TRADES (
    ...
    ACCOUNT_ID VARCHAR(30) NOT NULL COMMENT 'Investment account used for settlement (References {{ crm_raw }}.ACCI_RAW_TB_ACCOUNTS.ACCOUNT_ID where ACCOUNT_TYPE = ''INVESTMENT'')',
    ...
    CONSTRAINT PK_{{ eqt_raw }}.EQTI_RAW_TB_TRADES PRIMARY KEY (TRADE_ID),
    CONSTRAINT FK_EQTI_RAW_TB_EQUITY_TRADES__{{ crm_raw }}.ACCI_RAW_TB_ACCOUNTS FOREIGN KEY (ACCOUNT_ID) REFERENCES {{ db }}.{{ crm_raw }}.ACCI_RAW_TB_ACCOUNTS(ACCOUNT_ID)
)
CHANGE_TRACKING = TRUE
COMMENT = 'Equity trades via FIX protocol with CHF as base currency. Uses INVESTMENT accounts from {{ crm_raw }}.ACCI_RAW_TB_ACCOUNTS. Signed amounts: positive for purchases, negative for sales.';
...
AS
    ...
    FROM @{{ eqt_raw }}.EQTI_RAW_ST_TRADES
    PATTERN = '.*trades.*\.csv'
...
AS
    CALL EQTI_RAW_SP_CLEANUP_STAGE_KEEP_LAST_N('{{ eqt_raw }}.EQTI_RAW_ST_TRADES', 5);
