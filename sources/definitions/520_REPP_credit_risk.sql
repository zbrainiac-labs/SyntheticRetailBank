...
LEFT JOIN {{ db }}.{{ rep_agg }}.REPP_AGG_DT_ANOMALY_ANALYSIS t ON c.CUSTOMER_ID = t.CUSTOMER_ID
LEFT JOIN {{ db }}.{{ pay_agg }}.PAYA_AGG_DT_ACCOUNT_BALANCES b ON c.CUSTOMER_ID = b.CUSTOMER_ID;
...
FROM {{ db }}.{{ rep_agg }}.REPP_AGG_DT_IRB_CUSTOMER_RATINGS r
...
FROM {{ db }}.{{ rep_agg }}.REPP_AGG_DT_IRB_PORTFOLIO_METRICS;
...
FROM {{ db }}.{{ rep_agg }}.REPP_AGG_DT_IRB_CUSTOMER_RATINGS r
LEFT JOIN {{ db }}.{{ rep_agg }}.REPP_AGG_DT_CUSTOMER_RATING_HISTORY h
