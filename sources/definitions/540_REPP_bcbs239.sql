DEFINE DYNAMIC TABLE {{ db }}.{{ rep_agg }}.REPP_AGG_DT_BCBS239_RISK_AGGREGATION(
    ...
    CUSTOMER_ID VARCHAR(30) WITH TAG ({{ db }}.PUBLIC.SENSITIVITY_LEVEL='top_secret') COMMENT 'Unique customer identifier for individual risk exposure tracking',
    TOTAL_EXPOSURE_CHF DECIMAL(38,2) WITH TAG ({{ db }}.PUBLIC.SENSITIVITY_LEVEL='restricted') COMMENT 'Total risk exposure amount in CHF for capital adequacy calculations',
    TOTAL_CAPITAL_REQUIREMENT_CHF DECIMAL(38,2) WITH TAG ({{ db }}.PUBLIC.SENSITIVITY_LEVEL='restricted') COMMENT 'Total capital requirement in CHF for Basel III compliance monitoring',
    ...
    MAX_SINGLE_EXPOSURE_CHF DECIMAL(38,2) WITH TAG ({{ db }}.PUBLIC.SENSITIVITY_LEVEL='restricted') COMMENT 'Maximum single customer exposure for concentration risk monitoring',
    EXPOSURE_VOLATILITY_CHF DECIMAL(38,2) WITH TAG ({{ db }}.PUBLIC.SENSITIVITY_LEVEL='restricted') COMMENT 'Standard deviation of exposures for risk volatility assessment',
    ...
    AVG_EXPOSURE_PER_CUSTOMER_CHF DECIMAL(38,2) WITH TAG ({{ db }}.PUBLIC.SENSITIVITY_LEVEL='restricted') COMMENT 'Average exposure per customer for risk distribution analysis',
...
DEFINE DYNAMIC TABLE {{ db }}.{{ rep_agg }}.REPP_AGG_DT_BCBS239_EXECUTIVE_DASHBOARD(
    TOTAL_EXPOSURE_CHF DECIMAL(38,2) WITH TAG ({{ db }}.PUBLIC.SENSITIVITY_LEVEL='restricted') COMMENT 'Total portfolio risk exposure in CHF for executive risk monitoring',
    TOTAL_CAPITAL_REQUIREMENT_CHF DECIMAL(38,2) WITH TAG ({{ db }}.PUBLIC.SENSITIVITY_LEVEL='restricted') COMMENT 'Total regulatory capital requirement in CHF for Basel III compliance',
    ...
    CREDIT_RISK_EXPOSURE_CHF DECIMAL(38,2) WITH TAG ({{ db }}.PUBLIC.SENSITIVITY_LEVEL='restricted') COMMENT 'Credit risk exposure amount in CHF for risk type analysis',
    MARKET_RISK_EXPOSURE_CHF DECIMAL(38,2) WITH TAG ({{ db }}.PUBLIC.SENSITIVITY_LEVEL='restricted') COMMENT 'Market risk exposure amount in CHF for trading risk monitoring',
    OPERATIONAL_RISK_EXPOSURE_CHF DECIMAL(38,2) WITH TAG ({{ db }}.PUBLIC.SENSITIVITY_LEVEL='restricted') COMMENT 'Operational risk exposure amount in CHF for operational risk management',
    LIQUIDITY_RISK_EXPOSURE_CHF DECIMAL(38,2) WITH TAG ({{ db }}.PUBLIC.SENSITIVITY_LEVEL='restricted') COMMENT 'Liquidity risk exposure amount in CHF for treasury risk monitoring',
    ...
    REGULATORY_CAPITAL_BUFFER_CHF DECIMAL(38,2) WITH TAG ({{ db }}.PUBLIC.SENSITIVITY_LEVEL='restricted') COMMENT 'Regulatory capital buffer amount in CHF for stress testing',
...
DEFINE DYNAMIC TABLE {{ db }}.{{ rep_agg }}.REPP_AGG_DT_BCBS239_REGULATORY_REPORTING(
    ...
    TOTAL_RISK_EXPOSURE_CHF DECIMAL(38,2) WITH TAG ({{ db }}.PUBLIC.SENSITIVITY_LEVEL='restricted') COMMENT 'Total risk exposure in CHF for regulatory risk reporting',
    TOTAL_CAPITAL_REQUIREMENT_CHF DECIMAL(38,2) WITH TAG ({{ db }}.PUBLIC.SENSITIVITY_LEVEL='restricted') COMMENT 'Total capital requirement in CHF for regulatory capital reporting',
...
DEFINE DYNAMIC TABLE {{ db }}.{{ rep_agg }}.REPP_AGG_DT_BCBS239_RISK_CONCENTRATION(
    CUSTOMER_ID VARCHAR(30) WITH TAG ({{ db }}.PUBLIC.SENSITIVITY_LEVEL='top_secret') COMMENT 'Unique customer identifier for concentration risk tracking',
    CUSTOMER_NAME VARCHAR(100) WITH TAG ({{ db }}.PUBLIC.SENSITIVITY_LEVEL='top_secret') COMMENT 'Customer name for concentration risk reporting',
    ...
    TOTAL_EXPOSURE_CHF DECIMAL(38,2) WITH TAG ({{ db }}.PUBLIC.SENSITIVITY_LEVEL='restricted') COMMENT 'Total customer exposure in CHF for concentration risk assessment',
...
DEFINE DYNAMIC TABLE {{ db }}.{{ rep_agg }}.REPP_AGG_DT_BCBS239_RISK_LIMITS(
    ...
    CURRENT_EXPOSURE_CHF DECIMAL(38,2) WITH TAG ({{ db }}.PUBLIC.SENSITIVITY_LEVEL='restricted') COMMENT 'Current risk exposure in CHF for limit utilization monitoring',
    RISK_LIMIT_CHF DECIMAL(38,2) WITH TAG ({{ db }}.PUBLIC.SENSITIVITY_LEVEL='restricted') COMMENT 'Risk limit amount in CHF for limit monitoring',
    UTILIZATION_PERCENT DECIMAL(10,2) COMMENT 'Limit utilization percentage for risk management',
    REMAINING_LIMIT_CHF DECIMAL(38,2) WITH TAG ({{ db }}.PUBLIC.SENSITIVITY_LEVEL='restricted') COMMENT 'Remaining limit capacity in CHF for risk management',
