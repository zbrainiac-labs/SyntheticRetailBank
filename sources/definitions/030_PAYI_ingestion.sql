...
DEFINE TABLE {{ db }}.{{ pay_raw }}.PAYI_RAW_TB_TRANSACTIONS (
    ...
    ACCOUNT_ID VARCHAR(30) NOT NULL COMMENT 'Reference to account ID in {{ crm_raw }}.ACCI_RAW_TB_ACCOUNTS',
    ...
    CONSTRAINT PK_{{ pay_raw }}.PAYI_RAW_TB_TRANSACTIONS PRIMARY KEY (TRANSACTION_ID),
    CONSTRAINT FK_{{ pay_raw }}.PAYI_RAW_TB_TRANSACTIONS__{{ crm_raw }}.ACCI_RAW_TB_ACCOUNTS FOREIGN KEY (ACCOUNT_ID) REFERENCES {{ db }}.{{ crm_raw }}.ACCI_RAW_TB_ACCOUNTS (ACCOUNT_ID)
)
...
AS
    COPY INTO {{ pay_raw }}.PAYI_RAW_TB_TRANSACTIONS (BOOKING_DATE, VALUE_DATE, TRANSACTION_ID, ACCOUNT_ID, AMOUNT, CURRENCY, BASE_AMOUNT, BASE_CURRENCY, FX_RATE, COUNTERPARTY_ACCOUNT, DESCRIPTION)
    FROM @{{ pay_raw }}.PAYI_RAW_ST_TRANSACTIONS
...
AS
    CALL PAYI_RAW_SP_CLEANUP_STAGE_KEEP_LAST_N('{{ pay_raw }}.PAYI_RAW_ST_TRANSACTIONS', 5);
