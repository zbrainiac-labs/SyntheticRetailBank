...
AS
    COPY INTO ICGI_RAW_TB_SWIFT_MESSAGES (FILE_NAME, RAW_XML)
    FROM (
        SELECT 
...
DEFINE TASK {{ db }}.{{ pay_raw }}.ICGI_RAW_TK_CLEANUP_AFTER_LOAD_SWIFT_MESSAGES
    USER_TASK_MANAGED_INITIAL_WAREHOUSE_SIZE = 'XSMALL'
    COMMENT = 'Automated stage cleanup AFTER {{ db }}.{{ pay_raw }}.SWIFT message data load. Keeps last 5 files to manage storage costs.'
